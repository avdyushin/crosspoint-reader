...
BibleSection::~BibleSection() { suspendBuild(); }

uint32_t BibleSection::onPageComplete(std::unique_ptr<Page> page) {
  if (!file) {
...
void BibleSection::writeSectionFileHeader(const ReaderRenderSpec& spec) {
  if (!file) {
...
bool BibleSection::loadSectionFile(const ReaderRenderSpec& spec) {
  if (!Storage.openFileForRead("SCT", cacheFilePath, file)) {
    return false;
  }
...
bool BibleSection::clearCache() const {
  ...
  if (!Storage.remove(cacheFilePath.c_str())) {
...
bool BibleSection::createSectionFile(const ReaderRenderSpec& spec, const std::function<void()>& popupFn) {
...
bool BibleSection::startBuild(const ReaderRenderSpec& spec, const std::function<void()>& popupFn) {
  if (build_) {
  ...
  ctx->parser = makeUniqueNoThrow<ChapterHtmlSlimParser>(
      nullptr, ctxPtr->parsePath, renderer, spec.fontId, spec.lineCompression, spec.extraParagraphSpacing,
      ...
      spec.embeddedStyle, "", "", spec.imageRendering, std::vector<std::string>{}, popupFn);
  if (!ctx->parser) {
...
bool BibleSection::buildSomeMore(const int maxPages) {
...
bool BibleSection::hasHtmlCache() const {
  // const std::string htmlPath = epub->getCachePath() + "/html/" + std::to_string(spineIndex) + ".html";
  // return Storage.exists(htmlPath.c_str());
  return true;
}

std::optional<uint16_t> BibleSection::findAnchorDuringBuild(const std::string& anchor) const {
...
std::optional<uint16_t> BibleSection::findAnchor(const std::string& anchor) const {
...
uint16_t BibleSection::estimatedTotalPages() const {
...
bool BibleSection::commitBuildFile(const uint8_t version, const uint32_t bytesConsumed, const uint32_t totalBytes) {
  ...
  if (Storage.exists(cacheFilePath.c_str())) {
    Storage.remove(cacheFilePath.c_str());
  }
  if (!Storage.rename(binTmpPath().c_str(), cacheFilePath.c_str())) {
...
bool BibleSection::finalizeBuild() {
...
void BibleSection::suspendBuild() {
  if (!build_) return;
...
void BibleSection::abandonBuild() {
  if (!build_) return;
  if (build_->parser) build_->parser->abortParse();
  // if (build_->cssParser) build_->cssParser->clear();
  if (file) {
...
std::unique_ptr<Page> BibleSection::loadPageDuringBuild(const int page) {
...
std::unique_ptr<Page> BibleSection::loadPageAt(const int page) const {
  HalFile f;
  if (!Storage.openFileForRead("SCT", cacheFilePath, f)) {
    return nullptr;
  }
...
std::unique_ptr<Page> BibleSection::loadPage(const int page) {
  if (page < 0) {
    return nullptr;
  }
...
std::string BibleSection::getTextFromSectionFile() {
  std::string fullText;
...
std::optional<uint16_t> BibleSection::getCachedPageCount() const {
  HalFile f;
  if (!Storage.openFileForRead("SCT", cacheFilePath, f)) {
    return std::nullopt;
  }
...
std::optional<uint16_t> BibleSection::getPageForAnchor(const std::string& anchor) const {
  HalFile f;
  if (!Storage.openFileForRead("SCT", cacheFilePath, f)) {
    return std::nullopt;
  }
...
std::optional<uint16_t> BibleSection::getPageForParagraphIndex(const uint16_t pIndex) const {
  HalFile f;
  if (!Storage.openFileForRead("SCT", cacheFilePath, f)) {
    return std::nullopt;
  }
...
std::optional<uint16_t> BibleSection::getParagraphIndexForPage(const uint16_t page) const {
  HalFile f;
  if (!Storage.openFileForRead("SCT", cacheFilePath, f)) {
    return std::nullopt;
  }
...
std::optional<uint16_t> BibleSection::getPageForListItemIndex(const uint16_t liIndex) const {
  HalFile f;
  if (!Storage.openFileForRead("SCT", cacheFilePath, f)) {
    return std::nullopt;
  }
...
std::optional<uint32_t> BibleSection::getVisibleTextOffsetForPage(const uint16_t page) const {
  if (build_ && page < build_->lut.size()) {
    return build_->lut[page].visibleTextOffset;
  }

  HalFile f;
  if (!Storage.openFileForRead("SCT", cacheFilePath, f) || f.size() < HEADER_SIZE) {
    return std::nullopt;
  }
...
std::optional<uint16_t> BibleSection::getPageForVisibleTextOffset(const uint32_t offset,
